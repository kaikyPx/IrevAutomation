...
export default function OddsScannerPage() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/oddsscanner?date_from=${dateFrom}&date_to=${dateTo}`);
      const result = await response.json();
      if (result.error) throw new Error(result.error);
      
      let resData = result.data || [];
      
      // Filtrar apenas registros que contenham '#' no nome (Source) E que tenham valores (não zerados)
      resData = resData.filter((item: OddsScannerData) => {
        const source = item.source || '';
        const hasHash = source.includes('#');
        const hasValue = item.registrations > 0 || 
                         item.ftds > 0 || 
                         item.cpa > 0 || 
                         item.deposits > 0 || 
                         item.ngr !== 0;
  ...
  const handleExport = () => {
    ...
    exportToExcel(csvData, headers, `oddsscanner_export_${dateFrom}_to_${dateTo}`);
  };
    ...
    <div className="space-y-12 animate-in fade-in duration-700">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 pb-6 border-b border-white/5">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-white uppercase tracking-wider">OddsScanner <span className="text-orange-500">Performance</span></h1>
          <p className="text-white/40 text-sm italic font-medium">Métricas consolidadas por Casa de Apostas via API.</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-4">
          ...
          <button onClick={handleExport} className="flex items-center gap-2 px-4 py-3 glass rounded-xl text-white/60 hover:text-white hover:bg-white/10 transition-all active:scale-95 group border border-white/5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {[
          ...
          { label: `Depósitos (${totals.currency})`, value: `${totals.currency === 'EUR' ? '€' :totals.currency === 'BRL' ? 'R$' : '$'} ${totals.deposits.toFixed(2)}`, icon: Wallet, color: "text-blue-400" },
          { label: `NGR (${totals.currency})`, value: `${totals.currency === 'EUR' ? '€' : totals.currency === 'BRL' ? 'R$' : '$'} ${totals.ngr.toFixed(2)}`, icon: TrendingUp, color: (totals.ngr < 0 ? 'text-red-400' : 'text-emerald-400') },
        ].map((stat, i) => (
            ...
            <tbody className="divide-y divide-white/5 text-white/70">
              {filteredData.map((row: any, i) => (
                <tr key={i} className={`hover:bg-white/5 transition-colors ${!row.affiliateId ? 'bg-red-500/5 opacity-80' : ''}`}>
                  ...
                  <td className="p-4 px-6 text-right text-blue-400 font-medium">{row.currency === 'BRL' ? 'R$' : '$'} {row.deposits.toFixed(2)}</td>
                  <td className={`p-4 px-6 text-right font-bold ${row.ngr < 0 ? 'text-red-400' : 'text-emerald-400'}`}>{row.currency === 'BRL' ? 'R$' : '$'} {row.ngr.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
